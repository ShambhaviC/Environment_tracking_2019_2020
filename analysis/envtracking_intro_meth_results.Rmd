...
fig_caption: yes
>>>>>>> b0b37ba89a8b318b108b3c253439413edef11f33
---
<style>
body {
text-align: justify}
...
#Set the working directory just once
knitr::opts_knit$set(root.dir = "/Users/shambhavi/Google Drive/Experiments & Data/Environmental Tracking - Berlin_2019/Rate of environmental change/CSVs")
>>>>>>> b0b37ba89a8b318b108b3c253439413edef11f33
```
...
# Results and summary figures
Faced with two options, one of which gives a stable output and the other a temporally varying output, a bat can change two aspects of its behaviour: 

* Where it makes a visit, i.e., to which option 

* When it makes a visit

A bat could different strategies to exploit its environment by changing when and where it pays visits. 
The following figures explore these aspects of the data. 
...
<<<<<<< HEAD
=======
### Is there a bout structure in the distribution of inter-visit intervals? 

Previous data gathered from wild bats (Nachev et al., 2019) indicated that bats showed a 'bout' structure in the distribution of IVIs. There is a bimodal structure to the distribution: the shorter IVIs occur within a foraging bout when the bat is making visits in rapid succession; the larger IVIs occur between foraging bouts. The wild bats had a clear 'bout break criterion' of 120 seconds: IVIs smaller than that were judged as 'within-bout' and IVIs larger than that were judged as 'between-bout'. 
The distribution of the IVIs for all the bats on all the days is given in Figure 14. These are the same data as those plotted in Figure 13 except they are not overlapping.  

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.width = 17, fig.height = 9, fig.cap = "Figure 14: Distribution of visits for all the bats in each of the 4 conditions"}
Main %>%
  ungroup() %>%
  #filter (Tracking == "tracker") %>%
  #group_by(Period, Tracking, IdLabel) %>%
  ggplot(aes(interval)) +
  geom_density() +
  facet_grid(Period ~ IdLabel) +
  scale_x_log10() +
...
To investigate whether there is a bout structure, the data were explored to see if the overall distribution could be separated into two component distributions. 
[Note that help is needed to change the figure so that the medians and not the means of the distributions are marked]

```{r echo=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.width = 17, fig.height = 9, fig.cap = "Figure 15: The bout structure of the inter-visit intervals. The vertical lines mark the means of the two distributions"}
if(!require(flexmix)){install.packages('flexmix')}
library(flexmix)

n <- 2 #set this to set the range of the potential number of clusters
mix_comp <- function(x) {
  fit_mixture <- stepFlexmix(x ~ 1, # the means and sds are constant data = gender, # the data frame
                             k = 1:n,  #range of the number of clusters
                             nrep = 15,
                             model = FLXMCnorm1(), # univariate Gaussian
                             control = list(tol = 1e-15, # tolerance for EM stop 
                                            #verbose = 1, # show partial results
                                            iter = 1e4))
  best_fit <- getModel(fit_mixture, which = "BIC")
  priors  <- data.frame(length(prior(best_fit))) %>%
    rename(components = 1) %>%
    mutate(kind = "component") %>%
    unite("component", 1:2)

  parameters <- as.vector(parameters(best_fit)) %>%
    as.data.frame() %>%
    rename(components = 1) %>%
    mutate(components = round(components, digits = 2),
           rown = 1:n(),
           kind = ifelse(rown%%2 == 1, "mean", "sd"),
           row = rep(1:(max(rown)/2), each = 2)) %>%
    select(components, row, kind) %>%
    unite("kind", 2:3, sep = "") %>%
    unite("component",  1:2, sep = "_")
  components <- list((bind_rows(priors, parameters)))
}

IVIs <- Main %>%
  select(Period, IdLabel, interval) %>%
  filter(interval != "NA") %>%
  group_by(Period, IdLabel) %>%
  as.tibble()
...
mix_dist <- IVIs %>%
  group_by(Period, IdLabel)  %>%
  summarise(components = mix_comp(interval)) %>%
  mutate(components = map(components, as_tibble)) %>%
  unnest() %>%
  separate(component, c("component", "kind"), sep = "_", convert = TRUE) %>%
  select(Period, IdLabel, kind, component) %>%
  spread(kind, component, fill = 0, drop = FALSE) %>%
  mutate_at(vars(3:ncol(.)), funs(as.numeric))

IVIs_complete <- left_join(IVIs, mix_dist, by = c("Period", "IdLabel"))
...
IVIs_complete %>%
  ...
  facet_grid(Period ~ IdLabel) +
  scale_x_log10() +
  geom_vline(mapping = aes(xintercept = IVIs_complete$`1mean`), color = "blue", linetype = 2) +
  geom_vline(mapping = aes(xintercept = IVIs_complete$`2mean`), color = "red", linetype = 2) +
  #geom_vline(mapping = aes(xintercept = IVIs_complete$`3mean`), color = "green", linetype = 2) +
  theme_bw() +
  xlab("intervisit intervals [s]") +
  scale_fill_viridis_d()
